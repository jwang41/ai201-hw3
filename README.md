...
1. **Address imbalance directly** — class-weighted loss, oversampling the
   `Special` class, or lowering the decision threshold on the `Special`
   probability. This is the single highest-leverage change for lifting recall.
2. **Clean the data** — drop or repair rows with empty `text`; consider using the
   `name` field as an extra signal.
3. **Tighten the label definition** and re-check the noisiest examples, since some
   "errors" are really annotation disagreements.
4. **Re-run the baseline within rate limits** — add retry/back-off on HTTP 429,
...
Local dependencies for the supporting scripts:
```bash
...
├── takemeter_starter.ipynb      # training + evaluation pipeline
...
└── evaluation_results.json      # generated metrics
```
